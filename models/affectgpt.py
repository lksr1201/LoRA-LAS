...
class AffectGPT(Blip2Base):
    ...
    def __init__(
        ...
        '''
        # 替换为LoRAN
        from my_affectgpt.models.loran import apply_loran
        
        # 应用LoRAN
        lora_r = 16  # 低秩维度
        lora_alpha = 32  # 缩放因子
        lora_dropout = 0.1  # Dropout率
        sinter_amplitude = 5e-5  # Sinter幅度参数
        sinter_omega = 1e4  # Sinter频率参数
        frozen_llm = True  # 是否冻结LLM权重
        
        self.llama_model = apply_loran(
            self.llama_model,
            lora_r=lora_r,
            lora_alpha=lora_alpha,
            lora_dropout=lora_dropout,
            sinter_amplitude=sinter_amplitude,
            sinter_omega=sinter_omega,
            frozen_llm=frozen_llm
        )
        '''
        # 替换为LoRAF
        from my_affectgpt.models.lornf import apply_loran
    ...
    def forward(self, samples):
        ...
        try:
            ...
            try:
                ...
                # 尝试其他可能的结构
                temp_input_embedding = self.llama_model.embed_tokens(temp_input_ids)
